...
function escapeSpecialChars(value) {
  ...
  if(value !=  tempEl.innerHTML)
  console.log("::escaping", value)
  return tempEl.innerHTML
//   return JSON.stringify(tempEl.innerHTML);
}

export default function HTMLtoJSX(config) {
  this.config = config || {};
    ...
    switch (attribute.name) {
      ...
      default:
        ...
        if (isNumeric(attribute.value)) {
		  ...
		  if(attribute.value.replaceAll(/"/gm, '&quot;') !== attribute.value){
			result += '={\'' + attribute.value + '\'}'
		  }else{
			  result += '="' + attribute.value.replaceAll(/"/gm, '&quot;') + '"';
		  }
        }
        return result;
    }
