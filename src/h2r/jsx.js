import {parse as parseJSX} from "babylon"
import traverse from "babel-traverse"
import HTMLtoJSX from "./html2jsx"
  ...
  traverse(ast, {
    JSXElement: function(p) {
      if (p.node.openingElement.attributes.length > 0) {
        ...
        childPublicAttrs = p.node.openingElement.attributes.filter(function(attr) {
          return attr.name.hasOwnProperty("namespace") && attr.name.namespace.name === "public";
...
export default function toJsxAST(components) {
  return Object.keys(components).reduce(function(cs, name) {
    var outs = components[name].map((s) => html2jsx.convert(s)).map((jsx) => parseJSX(jsx, { plugins: ["jsx"] })).map((ast) => childrenToComponents(ast));
